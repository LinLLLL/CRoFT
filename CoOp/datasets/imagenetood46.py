...
class ImageNetOOD46(DatasetBase):
    ...
    def __init__(self, cfg):
        ...
        # test_id = self.generate_fewshot_dataset(test_id, num_shots=4)   ##############
        ...
        variables={'imagenet_v2': imagenet_v2,
                   'imagenet_s': imagenet_s,
                   'imagenet_a': imagenet_a,
                   'imagenet_r': imagenet_r,
                   'test_id': test_id}
        test_set = variables[cfg.test_ood]

        super().__init__(train_x=train, val=val, test=test_set, ood_test=test_ood)
        # super().__init__(train_x=train, val=imagenet_v2, test=test_id, ood_test=test_ood)
    ...
    def read_data_variation(self, classnames, image_dir):
        ...
        for label, folder in enumerate(folders):
            ...
            for imname in imnames:
                ...
                if classname in self.base_classes:
                    items.append(item)
        return items
